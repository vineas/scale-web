...
export default function DaftarTransporter() {

    const handlePrint = async () => {
        window.open(`/daftar-transporter-print`, "_blank");
    };

    const exportToExcel = (data: Transporter[], filename: string) => {
        const formatted = data.map((item) => ({
            Kode_Transporter: item.kode_transporter,
            Nama_Transporter: item.nama_transporter,
        }));
    ...
    const handleExport = async () => {
        ...
        exportToExcel(data as unknown as Transporter[], "daftar-transporter");
    };
            ...
            <div className="p-6 bg-white shadow-sm rounded-lg">
                ...
                <div className="flex items-center justify-center">
                    <div className="grid grid-cols-1 lg:grid-cols-7 md:grid-cols-1 xl:grid-cols-1 text-center">
                        ...
                        <h1 className="text-2xl font-bold text-gray-900">Transporter</h1>
                        <div></div>
                        <div></div>
                        <div></div>
                    </div>
